...
COMMANDS := [?]Command {
	// --- Filesystem ---------------------------------------------------------
	{
		...
		CAT_FS,
		"ls [-l] [-a] [path...]",
		"List directories or files. -l shows owner, size and permissions, -a includes dotfiles.",
		cmd_ls,
	},
	...
	{"tail", CAT_TEXT, "tail [-N] [-n N] [file]", "Print the last N lines (default 10).", cmd_tail},
	{
		...
		CAT_TEXT,
		"wc [-l] [-w] [-c] [file...]",
		"Count lines, words and bytes. A single flag prints just that number.",
		cmd_wc,
	},
	...
	{"ps", CAT_SYS, "ps [-a]", "List running processes. -a shows everyone's.", cmd_ps},
	{"jobs", CAT_SYS, "jobs", "List the background jobs you started.", cmd_jobs},
	{"kill", CAT_SYS, "kill <pid...>", "Stop a process you own.", cmd_kill},
	{"wait", CAT_SYS, "wait [pid...]", "Block until background jobs finish.", cmd_wait},
	{"sleep", CAT_SYS, "sleep <seconds>", "Do nothing for a while. Useful with &.", cmd_sleep},
	{
		"sh",
		CAT_SYS,
		"sh <script> [args...]",
		"Run a script from the filesystem. Arguments arrive as $1..$9.",
		cmd_sh,
	},
	{
		"test",
		CAT_SYS,
		"test <expression>",
		"Evaluate a condition for `if` and `while`. Also spelled [ ].",
		cmd_test,
	},
	{"[", CAT_SYS, "[ <expression> ]", "The same as test, in the usual brackets.", cmd_bracket},
	{"true", CAT_SYS, "true", "Succeed. Useful as a loop condition.", cmd_true},
	{"false", CAT_SYS, "false", "Fail. The opposite of true.", cmd_false},
	{"exit", CAT_SYS, "exit [status]", "End a running script, or log out when there is none.", cmd_exit},
